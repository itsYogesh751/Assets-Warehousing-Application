...
public class ViewPatentAssets extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        checkUserDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                Map<String, HashMap> mp = (Map<String, HashMap>) snapshot.child(username).child("asset").child("Intangible").child("Patent").getValue();
                StringBuilder j = new StringBuilder();
                ListView listasset = findViewById(R.id.bank_list);
                for (Map.Entry<String, HashMap> entry : mp.entrySet()) {
                    j = new StringBuilder(" Patent Application Number= " + entry.getValue().get("Patent Application Number") +
                            "\n Patent Number= " + entry.getValue().get("Patent Number") +
                            "\n date of grant= " + entry.getValue().get("date of grant")
                    +"\n Patent description= " + entry.getValue().get("Patent description") );
